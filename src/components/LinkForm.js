...
function LinkForm({ isModifyActive, buttonText }) {
  ...
  if (isModifyActive) {
    myObject = populatedObject
  } else {
    myObject = emptyObject
  }

  const [link, setLink] = useState(myObject);
  ...
  const handleSaveLink = async (e) => {
    ...
    try {
      ...
      if (isModifyActive) {
        response = await AuthLinkProvider.updatelink(form_data)
      } else {
        response = await AuthLinkProvider.saveLink(form_data)
      }
